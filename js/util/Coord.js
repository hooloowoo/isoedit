var coord = (function () {
    ...
    function samePoint(p0,p1,pe) {
        return (fequ(p0.x,p1.x,pe) && fequ(p0.y,p1.y,pe));
    }

    function isPointOnLine(p,l,pe) {
        var lp0=l.p0;
        var lp1=l.p1;
        var c1;
        var c2;
        ...
        var bb0=fgte(p.x,lp0.x,pe);
        var bb1=fgte(p.y,lp0.y,pe);
        var bb2=flte(p.x,lp1.x,pe);
        var bb3=flte(p.y,lp1.y,pe);
        var rb0=(bb0 && bb1 && bb2 && bb3);

        bb0=fgte(p.x,lp1.x,pe);
        bb1=fgte(p.y,lp1.y,pe);
        bb2=flte(p.x,lp0.x,pe);
        bb3=flte(p.y,lp0.y,pe);
        var rb1=(bb0 && bb1 && bb2 && bb3);

        bb0=fgte(p.x,lp0.x,pe);
        bb1=flte(p.y,lp0.y,pe);
        bb2=flte(p.x,lp1.x,pe);
        bb3=fgte(p.y,lp1.y,pe);
        var rb2=(bb0 && bb1 && bb2 && bb3);

        bb0=flte(p.x,lp0.x,pe);
        bb1=fgte(p.y,lp0.y,pe);
        bb2=fgte(p.x,lp1.x,pe);
        bb3=flte(p.y,lp1.y,pe);
        var rb3=(bb0 && bb1 && bb2 && bb3);

        var result=fequ(c1,c2) && (rb0 || rb1 || rb2 || rb3);
        return result;
    };
    ...
    */
    ...
    function linesOfArch(cp,r,b,e,dd)
    {
        var ddeg=(dd === undefined ? 1 : dd);
        var ret=[];
        if (b > e) e+=360;
        var lp=null;
        var i=b;
    ...
    function fequ(f0,f1,pe) {
        var e=(pe === undefined ? 0.001 : pe);
        return Math.abs(f0-f1) <= e;
    }

    function fgte(f0,f1,pe) {
        if (fequ(f0,f1,pe)) return true;
        return f0 > f1;
    }

    function flte(f0,f1,pe) {
        if (fequ(f0,f1,pe)) return true;
        return f0 < f1;
    }
    ...
    function distPointFromArc(cp,r,b,e,p) {
        var lns=linesOfArch(cp,r,b,e);
        var ret=distPointFromLine(lns[0],p);
        for(var i=1;i < lns.length;i++) {
            var d=distPointFromLine(lns[i],p);
            if (d.d < ret.d) ret=d;
        }
        return ret;
    }
    ...
    function isPointInRect(p,r) {
        return ((p.x >= r.p0.x) && (p.y >= r.p0.y) && (p.x < r.p1.x) && (p.y < r.p1.y));
    }

    function paralell(l,len,dir) {
        var llen=calcLen(l);
        var lp=perpendicular(l,len,dir);
        var pp=perpendicular({p0:lp.p1,p1:lp.p0},llen,dir);
        return pp;
    }

    function subline(l,lenToCut,base) {
        var basePoint = (base === undefined ? 0 : base);
        var xd=l.p1.x-l.p0.x;
        var yd=l.p1.y-l.p0.y;
        var llen=lenToCut/calcLen(l);
        var xsd=xd*llen;
        var ysd=yd*llen;
        var rp0=(basePoint === 0 ? l.p0 : {x:l.p0.x+xsd,y:l.p0.y+ysd});
        var rp1=(basePoint === 1 ? l.p1 : {x:l.p1.x-xsd,y:l.p1.y-ysd});

        var result={p0:rp0,p1:rp1};
        return result;
    }
    ...
    function distOfDegs(d0,d1) {
        var ret=Math.abs(d0-d1);
